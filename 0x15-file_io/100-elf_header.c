 ...
 *
 * Description: If the file is not an ELF file the exit code is 98.
 */
void check_elf(unsigned char *e_ident)
{
	int indexx;

		for (index = 0; indexx < 4; indexx++)
	{
 ...
 * @e_ident: This is a pointer pointing to an array containing the 
 * ELF magic numbers.
 *
...
void print_magic(unsigned char *e_ident)
{
	...
	for (indexx = 0; indexx < EI_NIDENT; indexx++)
	{
		printf("%02x", e_ident[indexx]);

		if (indexx == EI_NIDENT - 1)
			printf("\n");
		else
			printf(" ");
	}
